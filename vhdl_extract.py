...
class VhdlExtractor(object):
  def __init__(self, vector_types=set()):
    self.vector_types = set(('std_ulogic_vector', 'std_logic_vector',
  ...
  def parse_generic(self, glines):
    ...
    for l in glines:
      ...
      if m:
        pin_type = m.group(2)
        bus = True if pin_type.lower() in self.vector_types else False
